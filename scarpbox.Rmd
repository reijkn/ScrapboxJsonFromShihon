...
shihonToScrapbox <- function(){
  ...
  books_sub.df <- books.df %>%
    ...
    mutate(Update = if_else(is.na(Update), "20190101", Update)) %>%
    select(title = "タイトル", 
           ISBN = "ISBN", 
           author = "作者1", 
           author2 = "その他の作者",
           ...
           fileName = "備考2...19",
           "Update") %>%  #この順番になる？ なります．
    mutate(picURL = if_else(is.na(picURL), "h", picURL))
  ...
  books_sub.df <- books_sub.df %>% 
    mutate(ff = if_else(filePath < 2, 
                        if_else(is.na(fileName), "0", fileName),
                        str_c(filePath, fileName, sep = "/"))) %>% 
